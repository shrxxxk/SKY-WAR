...
start = pygame.image.load(path.join(img_dir,"start.png")).convert_alpha()
back = pygame.image.load(path.join(img_dir,"back.png")).convert_alpha()
exit = pygame.image.load(path.join(img_dir,"exit.png")).convert_alpha()
back = pygame.image.load(path.join(img_dir,"back.png")).convert_alpha()
replay = pygame.image.load(path.join(img_dir,"replay.png")).convert_alpha()
infoimg = pygame.image.load(path.join(img_dir,"infoimg.png")).convert_alpha()
jeda = pygame.image.load(path.join(img_dir,"jeda.png")).convert_alpha()
lanjut = pygame.image.load(path.join(img_dir,"lanjut.png")).convert_alpha()
kalah = pygame.image.load(path.join(img_dir,"gameoverimg.png")).convert_alpha()
...
def lastscore(last,x,y):
    label(str(last),x+100,y,30,white)
    lastscore(player.last,width/2-100,400)
...
def Button(img,x,y,width,height,command=None):
    ...
    if img_rect.collidepoint(mouse):
        screen.blit(img, (x, y))
    ...
    def shoot(self):
        ...
        if now - self.lastshoot > self.shootdelay:
            ...
            if self.bulletpower == 3:
                '''newbullet(self.rect.x+29,self.rect.y,math.pi/2) buat ditengah'''
                newbullet(self.rect.x-7, self.rect.y, math.pi / 2)
                newbullet(self.rect.x+70-8, self.rect.y, math.pi / 2)
                self.bulletpower == 20
...
class Enemy1(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image_org = pygame.transform.scale(enemy1img,(150,150))
...
class Enemy2(Enemy1):
    def __init__(self):
        super().__init__()
        self.image_org = pygame.transform.scale(enemy2img, (150, 150))
        self.image_org.set_colorkey(black)
        self.image = self.image_org.copy()
        self.rect = self.image.get_rect()
        self.rect.x = random.randrange(0, width - self.rect.width)
...
class Boss1(pygame.sprite.Sprite):
    ...
    def enemy_health(self,ph, fullph, x, y):
        container = pygame.Rect(x, y, width-150, 20)
        blood = pygame.Rect(x + 1, y + 1, width-150 * ph / fullph - 2, 20 - 2)
        pygame.draw.rect(screen, white, container, 1)
        pygame.draw.rect(screen, black, blood)
...
class Enemy3(Enemy1):
    def __init__(self):
        super().__init__()
        self.image_org = pygame.transform.scale(enemy3img, (70, 40))
        self.image_org.set_colorkey(black)
        self.image = self.image_org.copy()
        self.rect = self.image.get_rect()
        self.rect.x = random.randrange(0, width - self.rect.width)
...
def intro():
    ...
    while True:
        ...
        Button(exit,width-300,height/2+100,200,150,quitgame)
        Button(start,100,height/2+100,200,150,gameloop)
        Button(infoimg,width/2-100,height/2+100,200,150,info)
...
def info():
    ...
    while True:
        ...
        Button(off_music,width/2-40, 460, 40, 40,sound_off)
        Button(on_music,width/2-40, 500, 40, 40,sound_on)
        Button(exit,width-300,height/2+100,200,150,quitgame)
        Button(start,100,height/2+100,200,150,gameloop)
        Button(back,width/2-100,height/2+100,200,150,intro)
    ...
    while paused:
        for event in pygame.event.get():
            ...
            if event.type == [pygame.KEYDOWN]:
                if event.key == pygame.v:
                    paused = False
        screen.blit(infobackground, (0, 0))
        Button(jeda, width / 2 - 370, height/2 - 400, 700, 500, None)
        Button(lanjut, width / 2 - 100, height / 2 - 50, 220, 120, gameloop)
        Button(exit, width / 2 - 100, height / 2 + 30, 200, 150, quitgame)
...
def gameover():
    ...
    while True:
        ...
        Button(kalah, width / 2 - 400, height/2 - 300, 800, 600, None)
        Button(exit,width-700,height/2+100, 200, 150,quitgame)
        Button(replay,width / 2 - 300, height / 2 + 50, 300, 200,gameloop)
        label("SCORE ",width/2-150,height/2-150,50,red)
...
def gameloop():
    ...
    while True:
        ...
        for hit in hits:
            player.ph += random.randrange(30,70)
            healthup_sound.play()
        ...
        try:
            ...
            # boss3 die generate boss1 (nanti kalau boss 3 udah mati niatnya kita bakal finish)
            if boss3.ph <= 0:
                count1 += 2
                newbulletup()
                newspeedup()
        ...
        try:
            if boss1.drawph:
                boss1.enemy_health(boss1.ph, boss1.fullph, width - 200, 20)
        except:
            pass
        # draw boss2 ph
        try:
